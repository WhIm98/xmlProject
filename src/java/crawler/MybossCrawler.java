...
public class MybossCrawler extends BaseCrawler implements Runnable {
    ...
    public void run() {
        ...
        try {
            if(url.contains("do-bong-da")){
                String realPath = DataContants.getInstance().getYouSportLink(context) + url;
                url = realPath;
            }
            ...
            for (int i = 0; i < modelLink.size(); i++) {

                Thread pageCrawling = new Thread(new MybossEachPageCrawler(modelLink.get(i), categoryName, getContext()));
    ...
    private String getModelListDocument(BufferedReader reader) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            if (!isStart && line.contains("<div class=\"item-list\">")) {
                isStart = true;
            }
            if (isStart) {
                document += line.trim();
